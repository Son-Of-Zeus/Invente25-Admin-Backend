...
2. Staff authentication: allow only emails in the approved volunteer JSON
   file to sign up, store bcrypt password hashes in `verification`, generate
   each account's `volunteer_id`, and issue the agreed RS256 JWT. Derive
   receipt access from its roles.
...
5. Integration: configure the external database endpoint, Azure container,
   ...
   key, and the approved email JSON file. The participant frontend performs
   the PATCH to its own repository after this backend validates the upload.
